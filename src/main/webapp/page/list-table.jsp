    ...
    var vm = new Vue({
      ...
      data : {
    	  ...
    	  editDialog: {_name_cn:'', _name:'', 
    		  headers:[{text:'ID', value:'_id'}, {text:'列中文名', value:'_name_cn'}, {text:'列英文名', value:'_name'}, 
    			  {text:'数据类型', value:'_data_type'}, {text:'最大长度', value:'_max_len'}], 
    		  datas: []},
      },
    	  ...
    	  var url = '<%=request.getContextPath()%>/rest/tableprototype/table?dept-id=<%=request.getParameter("deptId")%>';
   		  axios.get(url).then(function (res) {
   			  ...
   			  for (var i = 0; i < d.length; ++i) {
       			  vm.tableDatas.push({_id: d[i]._id, _name: d[i]._name, _name_cn: d[i]._name_cn});
       		  }
       	  });
